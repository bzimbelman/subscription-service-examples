#!/usr/bin/env bash
# Smoke test: pods Ready, readiness probe green via Application Gateway.
...
step "Readiness probe green via App Gateway"
ip=$(kubectl -n "$NAMESPACE" get ingress -o jsonpath='{.items[0].status.loadBalancer.ingress[0].ip}')
if [ -z "$ip" ]; then
  fail "Ingress has no IP yet; AGIC still wiring?"
fi
...
echo
echo "OK: subscription-service is healthy on AKS (ingress IP ${ip})"
